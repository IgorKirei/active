...
var router = require('./router'),
    layers = require('./maintenance/layers'),
...
function Application() {
    ...
    /**
     * Add middleware layer for all queries, it can be your own solution or package of other developer
     *
     * @method useLayer
            ...
            dom.run(function() {
                that.router.parseQuery(req, res, function(err, found) {
                    if (found && found.callback) {
                        // add layers
                        async.each(that.layers, function(layer, next) {
                            layer(req, res, next);
                        }, function(err) {
