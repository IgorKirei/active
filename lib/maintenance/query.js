...
function Query() {
      ...
      case "DELETE":
        ...
        req.on("end", function() {
          if (/application\/json/.test(req.headers["content-type"])) {
            ...
            /application\/octet-stream/.test(req.headers["content-type"])
          ) {
  ...
  this.json = function(body, callback) {
    if (!Buffer.isBuffer(body)) {
      return callback(null, this.fields, this.files);
    }
    ...
    if (utils.validate.json(body)) {
      ...
      callback(new TypeError("invalid json"), this.fields, this.files);
    }
  };
  ...
  this.multipart = function(body, callback) {
    ...
    if (this.headers["content-type"].indexOf("boundary") < 0) {
      return callback(new Error("invalid headers, need boundary"));
    }
