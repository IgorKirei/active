...
function Application() {
  ...
  this.tune = settings => {
    ...
    if (settings.routing !== undefined) {
      if (["strict", "nonstrict"].includes(settings.routing)) {
        this.router.setupSettings("routing", settings.routing);
      } else {
        utils.showError(
          'tune() invalid routing settings, expected "strict" or "nonstrict"'
        );
      }
    }

    if (settings.cors !== undefined) {
      if (typeof settings.cors === "boolean") {
        this.settings.cors = settings.cors;
      } else {
        utils.showError("tune() invalid cors settings, expected boolean value");
      }
    }
    ...
    if (settings.debug !== undefined) {
      if (typeof settings.debug === "boolean") {
        ...
        utils.showError(
          "tune() invalid debug settings, expected boolean value"
        );
      }
    }
  };
